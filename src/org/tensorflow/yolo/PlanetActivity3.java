...
public class PlanetActivity3 extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState){
       ...
       }else if(AppSetting.progress==31) {
           // 지연 : 동화 미션을 수행한 상태
           ...
           r = new Runnable() {
               ...
               public void run() {
                   if (AppSetting.dp_bool) {
                       ...
                       img2.setImageResource(R.drawable.antenna_gray);
                       AppSetting.dp_bool = true;
                   }
                   handler.postDelayed(r, 200);
               }
       ...
       }else if(AppSetting.progress==32) {
           // 퀴즈 수행을 마친 단계
           // 동화 미션 클릭시 : BookActivity로 이동
           img.setOnTouchListener(new View.OnTouchListener() {
               @Override
       ...
       }else if(AppSetting.progress==40) {
           // 발음 미션을 완료한 단계
           // 모든 이미지 터치 가능
           // 미션 이미지 모두 color
           img.setImageResource(R.drawable.spaceship);
